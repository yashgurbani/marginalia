...
test('content saved maps follow journal reload/helper sync, refuse stale payloads and clear on exclusion/unmount/navigation', async t => {
  ...
  assert.equal(marks.get('marginalia-kept').ranges.length, 1); assert.equal(marks.get('marginalia-highlighted').ranges.length, 0);
  ...
  await message(packet()); assert.equal(marks.get('marginalia-highlighted').ranges[0].startContainer, source);
  await message({ type: 'highlight', document: identity.document, anchor });
  assert.equal(marks.has('marginalia-selection'), true); assert.equal(marks.get('marginalia-highlighted').ranges.length, 1);
  ...
  await journal.sync(async change => { helper.apply(change); }, async () => helper.list());
  await message(packet()); assert.equal(marks.get('marginalia-highlighted').ranges.length, 0); assert.equal(marks.get('marginalia-kept').ranges.length, 1);
