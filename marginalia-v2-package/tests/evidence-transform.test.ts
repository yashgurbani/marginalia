...
test('a fetch claim backed by an observed fetched record is supported and binds the record', () => {
...
test('two claims may share one observed source without double-counting the fetch', () => {
...
test('a mix of a genuine and a forged claim supports the genuine one and flags the forged one', () => {
  ...
  assert.match(result.headline ?? '', /1 of 2/);
});
