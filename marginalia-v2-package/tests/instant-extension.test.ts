...
test('background ready-help bridge authenticates source and options, forwards dismissal to the current content document', async t => {
  ...
  const browser = { windows: { get: async (id: number) => ({ id, type: 'normal', incognito: false }) }, runtime: { id: 'test', getURL: (path: string) => 'chrome-extension://test' + path, onMessage: event('message'), getContexts: async () => [{ contextType: 'SIDE_PANEL', tabId: -1, incognito: false, documentId: 'panelDoc', documentUrl: 'chrome-extension://test/panel.html', windowId: 1 }] }, alarms: { onAlarm: event('alarm') }, action: { onClicked: event('click') }, sidePanel: {},
  ...
  const result = await message({ type: 'surface', version: 1, action: 'auto-assist-dismiss', candidateId: 'c1', panelWindowId: 1 }, { id: 'test', origin: 'chrome-extension://test', documentId: 'panelDoc', url: 'chrome-extension://test/panel.html' });
