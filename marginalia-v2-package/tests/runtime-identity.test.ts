...
// Use the runtime's synchronous identity convention: Windows async realpath can expand 8.3 aliases.
test('zero-env discovery uses PATH native executable and ordinary home, never ambient managed home', async t => {
  ...
  assert.deepEqual(dedicatedRuntimeIdentity({ env, userHome: user }), {
    executable: realpathSync(executable), codexHome: realpathSync(join(user, '.codex')), homeMode: 'ordinary',
  });
...
test('explicit dedicated pair stays disjoint and invalid override never falls through to discovery', async t => {
  ...
  assert.deepEqual(dedicatedRuntimeIdentity({ env, userHome: user }), { executable: realpathSync(executable), codexHome: realpathSync(home) });
...
test('Windows npm discovery resolves the native optional package without running cmd or ps1 shims', { skip: process.platform !== 'win32' }, async t => {
  ...
  assert.equal(dedicatedRuntimeIdentity({ env: { PATH: root }, userHome: root })?.executable, realpathSync(executable));
});
