...
test('source-bound derivation keeps step boundaries and saved position through a request-free reopen', async t => {
  ...
  const mounted = mountReply(root as unknown as HTMLElement, accepted.saved.reply, {
    ...
    onStateChange: state => saveP13View(accepted, `p13-derive-view-${accepted.view.revision}`, state),
  });
  ...
  next.click(); await until(() => accepted.view.revision === 2); assert.match(steps.textContent, /2 of 3 steps revealed/);
  next.click(); await until(() => accepted.view.revision === 3); assert.match(steps.textContent, /3 of 3 steps revealed/);
  assert.equal(next.disabled, true); next.click(); assert.match(steps.textContent, /3 of 3 steps revealed/);
  previous.click(); await until(() => accepted.view.revision === 4); assert.match(steps.textContent, /2 of 3 steps revealed/);
  mounted.destroy();
