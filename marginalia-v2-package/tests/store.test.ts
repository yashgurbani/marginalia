...
test('T07 F8 export is thread-scoped, separates observed output from host facts and excludes account data', () => {
  ...
  try {
    ...
    for (const [jobId, threadId] of [['mine', keep.threadId], ['other-job', 'other-thread']]) {
      ...
      store.db.prepare(`INSERT INTO jobs VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(jobId, threadId, jobId, hash, hash, hash, 'app-server', 'model', 'structured-final', hash, 'shared-grant', 'outcome_unknown', 1, jobId + '-attempt', null, 'disconnected', now, now, JSON.stringify(context), '[]', 'SECRET_PROVISIONAL');
