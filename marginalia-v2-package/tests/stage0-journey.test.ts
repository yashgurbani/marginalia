...
test('Stage 0 selection, Simulate it, reviewed explicit send, saved grid, local slider and explicit solver preparation', async t => {
  ...
  try {
    ...
    api.select(mutation.anchor); button(e.root.querySelector('.m-selection-actions')!, 'Ask').click(); await api.drain();
