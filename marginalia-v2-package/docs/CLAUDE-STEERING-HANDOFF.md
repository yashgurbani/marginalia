...
| **T07 recovery continuation** | [Existing 6Pro chat](https://chatgpt.com/c/6aab93df-8f84-83eb-b0b7-d5780aef4e22), `codex/pro-t07-integration-checkpoint` | Active: reconcile continuation only to `1517433`, not the already integrated first slice. Owns reader contract, ReaderStore, journal and their tests. Device-version choice and SQLite-consistent backup/newer-schema refusal are the continuation. |
| **T08 asking module** | New 6Pro chat; URL pending chief dispatch | Authorized narrow next lane, not yet claimed dispatched. Own ONLY `ui/asking/**`, `tests/asking*.test.ts`, `docs/evidence/T08-asking/**` and the T08 build receipt. No mount, helper, persistence, journal, provider or entry-point edits. T05 alone mounts it. |
...
T07 published **`c9c2b2764e574ee670337ee3010e64930f9aaa5e`**, parent **`4552b65`**, independently remote-verified in the user's latest coordination receipt. It is an old-base continuation, not the new integration checkpoint. Native Luna Max verification of that exact commit is running separately, as reported by the user; **no pass is recorded**. This existing run is not relabelled as dynamically routed MCP evidence and is not authorization for new native implementation or a duplicate verification run.
...
The exact c9c2b27 run is reported in progress, not passed. Even a later pass on that old parent would not prove the rebased continuation on `1517433`, combined T05 conflict controls, migration safety on every platform, or product acceptance. Preserve the exact report and compare it to the eventual integration checkpoint before attributing results.
...
1. Retrieve only the existing T05, T06 and T07 outputs. Verify each parent, exact file allowlist, diff and claimed evidence. Reconcile T07 continuation without replaying its integrated first slice. Review T05's combined UI against C6 and the reported helper checkpoint; the helper report is useful but not transferable acceptance.
...
4. Chief may dispatch the narrow T08 module while current nonoverlapping lanes proceed. It uses existing host preparation, consent and reply interfaces, not a second store or transport. T05 mounts it after the agreed interface and its own checkpoint are reconciled. Local page definitions, selection, dismissal, denial and a note ending in '?' send no inference request. Only the explicit permitted action sends. Check frozen note versions, preview replacement, revoked pairing, duplicate submit and closed-mount callbacks.
