...
| R4 | Persist before deep execution; Working → provisional first frame → Ready; keep last good partial on failure; reader can continue. | T05, T06, T09, T18, T20 | **Partial.** P10 adds a bounded, labelled provisional frame with cancellation and late-output coverage in `tests/asking-flow.test.ts`. Full live-run acceptance and the complete failure/retention matrix remain open. |
| R5 | At most one clarification; otherwise a visible default; mid-build questions remain pending with a taken default. | T05, T09, T16 | **Partial.** P03 adds the unsure instruction bundle and one-question budget; `tests/unsure-skill.test.ts` covers local admission. A recorded real run remains pending. |
| R6 | Typed blocks only; cheapest sufficient path is kernel → in-envelope samples → saved-solver replay → explicit new model call; assumptions are editable versions; unsupported work is stated. | T03, T05, T09, T18, T20 | **Partial.** P04/P05 cover solver authoring and continuation; P09 covers bounded local assumption edits. `tests/solver-bindings.test.ts` and `tests/local-assumptions.test.ts` provide local evidence. Actual saved-solver recompute and observed confinement remain open under P06. The [whitepaper](sources/RESEARCH-WHITEPAPER-v3.md) remains the governing four-path contract. |
| R7 | Per-part origin/relation; host and model checks have different authority; failed checks are visible; unchecked headline is withheld. | T00, T03, T05, T09, T14, T18 | **Partial.** P12 binds exact local quote labels to host receipts and saved source identity; `tests/evidence-acceptance.test.ts` exercises reconciliation and rejection. Semantic claim support and live-provider proof remain separate open gates. |
...
| R14 | Notes have explicit frozen/changeable anchors, are launch scope, exportable, feed vocabulary; `?` offers Ask but never sends; replies quote note version. | T05, T07, T11, T16 | **Partial.** Reader note versions and plain-file export have local coverage; P16 adds BibTeX with captured metadata (`tests/library-export.test.ts`). P20 automatic vocabulary gathering and external import interoperability remain open. |
...
| R26 | Stable prefixed IDs preserve focus; explicit handoff; live-region Ready without focus theft; all states keyboard-reachable. | T05, T16, T18, T12 | **Partial.** P17 adds actual Chrome keyboard journeys at 360 px and 200% zoom, reduced-motion cases and slider announcements (`tests/reading-a11y-browser.test.ts`, `tests/renderer-a11y-browser.test.ts`). Native assistive-technology speech and full loaded-extension acceptance remain open. |
