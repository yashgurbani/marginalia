...
const mounted = mountReply(replyRoot, committedReply, {
  ...
  sampleGenerationRecords: recordsForThisCommittedReplyVersion,
...
Current-state sample interpolation requires both complete authored generation inputs and a matching host-owned `SampleGenerationRecord`, supplied separately from candidate content and keyed by samples block ID. The browser-safe readiness check binds the complete reply, referenced model computation, declared axes and fixed inputs, interpolation policy, forbidden regions and sample data. It also requires current non-axis inputs to equal the recorded fixed values. A matching digest establishes identity, not solver correctness; an imported record never claims execution. Missing, historical, malformed, stale or mismatched context leaves the authored grid readable but unavailable for current interpolation. Defaults are never invented as provenance.

Readiness runs asynchronously over detached snapshots. A parameter change first withdraws current sampled output, then starts a new local digest check. The renderer accepts a result only while its generation, canonical reply-and-parameter state key and mount lifetime remain current. It passes only the immutable block and normalized parameters returned by readiness into the bounded interpolation kernel. No network, model turn or saved solver runs on this path. Complete rectilinear geometry, error-evidence presence, strict envelope bounds and forbidden regions remain kernel requirements after provenance admission. Producer error evidence is displayed without certifying it.
